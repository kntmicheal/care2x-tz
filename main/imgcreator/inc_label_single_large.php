...
if ($location['room_nr'])
    $locstr.='-' . $location['room_nr'];
if ($location['bed_nr'])
    $locstr.=' ' . strtoupper(chr($location['bed_nr'] + 96));
...
if ($ttf_ok) {
    ...
    ImageTTFText($label, 16, 0, $lmargin, $tmargin + 56, $eblack, $arial, $result['name_last'] . ', ' . ucwords($result['name_first']));
    # Date of birth
    ...
    # Family name, first name
    ImageString($label, 5, 10, 40, $result['name_last'] . ', ' . ucwords($result['name_first']), $eblack);
    # Date of birth
...
if (!$child_img) {
    ...
    if (file_exists($root_path . 'main/imgcreator/gd_test_request_' . $subtarget . '.php'))
        include_once($root_path . 'main/imgcreator/gd_test_request_' . $subtarget . '.php');
    else
        Imagepng($label);
    /*   Imagepng($label); */
}
