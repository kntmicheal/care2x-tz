...
function PersonAddrs($args) {
    ...
    if (checkAccess($args[0])) {
        ...
        if ($buf) {
            if ($buf['addr_citytown_nr'])
                $buf['addr_citytown_name'] = $person->CityTownName($buf['addr_citytown_nr']);
            else
                $buf['addr_citytown_name'] = '';
            return $buf;
        }else {
            return new IXR_Error(1006, '_ERROR_NORESULT');
        }
    ...
    if (checkAccess($args[0])) {
        ...
        if ($buff = $db->Execute($sql)) {
            if ($rec_count = $buff->RecordCount()) {
                ...
                return new IXR_Error(1301, '_ERROR_SQL_NORESULT');
            }
        } else {
            return new IXR_Error(1300, '_ERROR_SQL_QUERY');
        }
    } else {
        return new IXR_Error($error['code'], $error['msg']);
    }
