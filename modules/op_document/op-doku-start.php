...
function createElement($item, $err, $f_size = 7, $mx = 5) {
    ...
    if ($mode == 'saveok') {
        ...
        if ($isTimeElement)
            $ret_str.= ' onKeyUp="setTime(this,\'' . $lang . '\')">';
        else
            $ret_str.='>';
    }
    return $ret_str;
...
if (!isset($dept) || empty($dept))
    if ($_COOKIE['ck_thispc_dept'])
        $dept = $_COOKIE['ck_thispc_dept'];
    else
        $dept = 'plop'; // default department is plop
...
if ($mode == 'search' || $mode == 'paginate') { # Filter the search and paginate modes
    ...
    if (empty($GLOBAL_CONFIG['pagin_patient_search_max_block_rows']))
        $pagen->setMaxCount(MAX_BLOCK_ROWS);# Last resort, use the default defined at the start of this page
    else
...
<!-- 
    ...
    {
        if (mode == 1)
            idx.filters.alpha.opacity = 100
        else
            idx.filters.alpha.opacity = 70;
    }
    ...
    ?>
        function chkForm(d) {
            if (d.op_date.value == "") {
                ...
                d.scrub_nurse.focus();
                return false;
            } else if (d.assistant.value == "") {
                alert("<?php echo $LDPlsEnterAssistant; ?>");
                d.scrub_nurse.focus();
                return false;
  ...
  <td colspan=6 height=1><img src="' . $root_path . 'gui/img/common/default/pixel.gif" border=0 width=1 height=1 align="absmiddle"></td>
                        ...
                        <table border="0">
                            <tr>
                                ...
                                <td class="prompt">
                                        <?php
                                        if ($mode == 'search')
                                            echo '<font color=maroon>' . $LDSorryNotFound . '</font>';
                                        else
	...
	<input name="operator" type="text" size="25" value="';
        ...
        ?>
                                ...
                                <tr <?php if ($mode == 'saveok') echo "bgcolor=#ffffff"; ?>>
                                    ...
                                    <td><FONT color="#000099">
                                        <?php
                                        echo '<b>' . ucwords($result['name_first']) . '</b>';
                                        ?>
                                    </td>
                                </tr>
                                <tr <?php if ($mode == 'saveok') echo "bgcolor=#ffffff"; ?>>
        ...
        switch ($result['status']) {
            case "stat": echo $LDStationary;
                break;
            case "amb": echo $LDAmbulant;
                break;
        }
        ?>
                                ...
                                <tr <?php if ($mode == 'saveok') echo "bgcolor=#ffffff"; ?>>
                                    ...
                                    <td><font color="#800000">
                                            ...
                                            if ($mode == 'saveok') {
                                                ...
                                                if ($opdoc[class_s])
                                                    echo "$opdoc[class_s] $LDMinor  &nbsp; ";
                                                if ($opdoc[class_m])
                                                    echo "$opdoc[class_m] $LDMiddle &nbsp; ";
                                                if ($opdoc[class_l])
                                                    echo "$opdoc[class_l] $LDMajor";
                                                echo " $LDOperation";
                                            }
                                            else {
                                                ?>
    ...
    ?>
                        ...
                        <p>
                            ...
                            echo createElement('scrub_nurse', $scrub_nurse, 14, 14);
                            echo '<p>';
        ...
        ?>
            ...
            <?php endif; ?>
                            ...
                            <input type="hidden" name="mode" value="<?php if ($mode == 'saveok') echo 'update';
        else echo 'save' ?>">
                            <input type="hidden" name="dept_nr" value="<?php echo $dept_nr ?>">
                            <input type="hidden" name="sid" value="<?php echo $sid ?>">
                            <input type="hidden" name="lang" value="<?php echo $lang ?>">
                            <input type="hidden" name="update" value="<?php if ($mode == 'update') echo '1' ?>">
                            <input type="hidden" name="pn" value="<?php if ($mode == 'match' && $rows == 1) echo $result['encounter_nr'];
        else echo $pn ?>">
