...
if (isset($_SESSION['sess_user_origin']))
...
<img ' . createComIcon($root_path, 'persons.gif', '0', 'top') . '><FONT  COLOR="' . $cfg['top_txtcolor'] . '"  SIZE=6  FACE="verdana"> <b>' . $buf . '</b></font>';
    ...
    <table width=100% border=0 cellpadding="0" cellspacing="0"> 
        <tr>
            <td colspan=3><?php
                if ($target == "personell_reg")
                    echo '<img ' . createLDImgSrc($root_path, 'add_employee_blue.gif', '0') . ' alt="' . $LDAdmit . '">';
                else {
                    echo'<a href="' . $thisfile . URL_APPEND . '&target=personell_reg"><img ' . createLDImgSrc($root_path, 'add_employee_gray.gif', '0') . ' alt="' . $LDAdmit . '"';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
                if ($target == "personell_search")
                    echo '<img ' . createLDImgSrc($root_path, 'src_emp_blu.gif', '0') . ' alt="' . $LDSearch . '">';
                else {
                    echo '<a href="' . $thisfile . URL_APPEND . '&target=personell_search"><img ' . createLDImgSrc($root_path, 'src_emp_gray.gif', '0') . ' alt="' . $LDSearch . '" ';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
                if ($target == "personell_listall")
                    echo '<img ' . createLDImgSrc($root_path, 'lista-blu.gif', '0') . ' alt="' . $LDSearch . '">';
                else {
                    echo '<a href="' . $thisfile . URL_APPEND . '&target=personell_listall"><img ' . createLDImgSrc($root_path, 'lista-gray.gif', '0') . ' alt="' . $LDSearch . '" ';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
                echo '<img src="' . $root_path . 'gui/img/common/default/pixel.gif" width=20>';
                if ($target == "person_reg")
                    echo '<img ' . createLDImgSrc($root_path, 'register_blue.gif', '0') . ' alt="' . $LDSearch . '">';
                else {
                    echo '<a href="' . $thisfile . URL_APPEND . '&target=person_reg"><img ' . createLDImgSrc($root_path, 'register_gray.gif', '0') . ' alt="' . $LDSearch . '" ';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
