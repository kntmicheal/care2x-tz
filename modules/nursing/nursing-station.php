...
//                $sBuffer = '<a href="javascript:popPic(\'' . $bed['pid'] . '\')">';
            ...
            if ($is_patient && ($bed['encounter_nr'] != "")) {
                ...
                if (isset($sln) && $sln)
                    $sFamNameBuffer = eregi_replace($sln, '<span style="background:yellow">' . ucfirst($sln) . '</span>', ucfirst($bed['name_last']));
                else
                    $sFamNameBuffer = ucfirst($bed['name_last']);
                ...
                if (isset($sfn) && $sfn)
                    $sNameBuffer = eregi_replace($sfn, '<span style="background:yellow">' . ucfirst($sln) . '</span>', ucfirst($bed['name_first']));
                else
                    $sNameBuffer = ucwords($bed['name_first']);
            }else {
                $smarty->assign('sFamilyName', '');
                $smarty->assign('sName', '');
                $smarty->assign('cComma', '');
...
//                print_r($dateArr);
    ...
    if ($waitlist_count && $edit) {
        while ($waitpatient = $waitlist->FetchRow()) {
            ...
            if ($waitpatient['current_ward_nr'] != $ward_nr)
                $buf2 = createComIcon($root_path, 'red_dot.gif', '0', '', TRUE);
            else
                $buf2 = createComIcon($root_path, 'green_dot.gif', '0', '', TRUE);
            $TP_WLIST_BLOCK.='<nobr><img ' . $buf2 . '><a href="javascript:assignWaiting(\'' . $waitpatient['encounter_nr'] . '\',\'' . $waitpatient['ward_id'] . '\')">';
            $TP_WLIST_BLOCK.='&nbsp;' . $waitpatient['name_last'] . ', ' . ucwords($waitpatient['name_first']) . ' ' . formatDate2Local($waitpatient['date_birth'], $date_format) . '</nobr></a><br>';
        }
    }else {
        $TP_WLIST_BLOCK = '&nbsp;';
    }
