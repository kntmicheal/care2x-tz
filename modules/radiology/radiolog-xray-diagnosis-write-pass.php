...
if ($retpath == 'read_diagnosis')
    $breakfile = 'radiolog-xray-diagnosis.php?sid=' . $sid . '&lang=' . $lang;
else
    $breakfile = "javascript:window.top.location.replace('radiolog-xray-javastart.php?sid=$sid&lang=$lang&mode=display1')";
        ...
        if ((($userid != NULL) || ($keyword != NULL)) && ($passtag != NULL)) {
            ...
            switch ($passtag) {
                case 1:$errbuf = $errbuf . $LDWrongEntry;
                    echo '<img ' . createLDImgSrc($root_path, 'cat-fe.gif', '0') . ' align=left>';
                    break;
                case 2:$errbuf = $errbuf . $LDNoAuth;
                    echo '<img ' . createLDImgSrc($root_path, 'cat-noacc.gif', '0') . ' align=left>';
                    break;
                default:$errbuf = $errbuf . $LDAuthLocked;
                    echo '<img ' . createLDImgSrc($root_path, 'cat-sperr.gif', '0') . ' align=left>';
            }
