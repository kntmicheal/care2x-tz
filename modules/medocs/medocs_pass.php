...
<BODY  onLoad="document.passwindow.userid.focus();" bgcolor=<?php echo $cfg['body_bgcolor']; ?>
    ...
    <P>
        ...
        if ($cfg['dhtml']) {
            switch ($target) {
                case 'entry':$buf = $LDMedocs;
                    break;
                case 'search':$buf = $LDMedocs;
                    break;
                case 'archiv':$buf = $LDMedocs;
                    break;
                default: $target = "entry";
                    $buf = $LDMedocs;
            }
        ...
        <tr>
            <td colspan=3><?php
                if ($target == "entry")
                    ...
                    echo'<a href="medocs_pass.php?sid=' . $sid . '&target=entry&lang=' . $lang . '"><img ' . createLDImgSrc($root_path, 'newdata-gray.gif', '0') . ' alt="' . $LDAdmit . '"';
                    if ($cfg['dhtml'])
                ...
                if ($target == "search")
                    ...
                    echo '<a href="medocs_pass.php?sid=' . $sid . '&target=search&lang=' . $lang . '"><img ' . createLDImgSrc($root_path, 'such-gray.gif', '0') . ' alt="' . $LDSearch . '" ';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
