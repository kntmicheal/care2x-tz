...
switch ($retpath) {
    case "menu": $rettarget = $root_path . 'main/op-doku.php' . URL_APPEND;
        break;
    case "qview": $rettarget = 'nursing-or-dienst-schnellsicht.php' . URL_APPEND . '&hilitedept=' . $dept_nr;
        break;
...
if ($pmonth == 1)
    $sBuffer = $sBuffer . '12' . '&pyear=' . ($pyear - 1) . '">';
else
    $sBuffer = $sBuffer . ($pmonth - 1) . '&pyear=' . $pyear . '">';
if ($pmonth == 1)
    $sBuffer = $sBuffer . $monat[12];
else
    $sBuffer = $sBuffer . $monat[$pmonth - 1];
...
$sBuffer = '<a href="' . $thisfile . URL_APPEND . '&retpath=' . $retpath . '&dept_nr=' . $dept_nr . '&pmonth=';
if ($pmonth == 12)
    $sBuffer = $sBuffer . '1' . '&pyear=' . ($pyear + 1) . '">';
else
    $sBuffer = $sBuffer . ($pmonth + 1) . '&pyear=' . $pyear . '">';
if ($pmonth == 12)
    $sBuffer = $sBuffer . $monat[1];
else
    $sBuffer = $sBuffer . $monat[$pmonth + 1];
...
for ($i = 1, $n = 0, $wd = $firstday; $i <= $maxdays; $i++, $n++, $wd++) {
    ...
    switch ($wd) {
        ...
        case 6: $backcolor = 'class="saturday"';
            break;
        case 0: $backcolor = 'class="sunday"';
            break;
