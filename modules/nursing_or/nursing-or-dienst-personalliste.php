...
if (!empty($ipath)) {
    switch ($ipath) {
        case 'menu': $breakfile = $root_path . 'main/op-doku.php' . URL_APPEND;
            break;
        case 'qview': $breakfile = "nursing-or-dienst-schnellsicht.php" . URL_APPEND . "&hilitedept=$dept_nr";
            break;
        case 'plan': $breakfile = "nursing-or-dienstplan-planen.php" . URL_APPEND . "&dept_nr=$dept_nr&pmonth=$pmonth&pyear=$pyear&retpath=$retpath";
            break;
...
<ul>
    ...
    while ($row = $nurses->FetchRow()) {
        ?>
                ...
                <td  class="v13" colspan=2><nobr>&nbsp;<?php echo ucwords($row['name_first']); ?></nobr></td>
...
if ($mode == 'search' || $mode == 'paginate') {
    ...
    if ($pers_obj->record_count) {
        ...
        while ($row = $search_result->FetchRow()) {
            ...
            echo "</td>";
            echo"<td>";
            echo "&nbsp;" . ucwords(ucwords($row['name_first']));
            echo "</td>";
            echo"<td>";
