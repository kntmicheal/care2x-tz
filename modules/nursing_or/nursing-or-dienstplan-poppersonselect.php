...
<BODY  LINK="navy" VLINK="navy" onLoad="if (window.focus)
            window.focus()" >
    ...
    <b>
        ...
        <?php
        if ($mode == "a")
            echo '<font color="#006666">' . $LDStandbyPerson . '</font>';
        else
            echo $LDOnCall;
        echo ' ' . $LDOn . '<br>';
    ...
    <p>
	    ...
	    <font face="verdana,arial" size=2>';
            ...
            while ($row = $nurses->FetchRow()) {
                echo '
	    <a href="#" onClick="addelem(\'' . $mode . $elemid . '\',\'h' . $mode . $elemid . '\',\'' . ucfirst($row['name_last']) . '\',\'' . ucwords(ucwords($row['name_first'])) . '\',\'' . $row['date_birth'] . '\',\'' . $row['personell_nr'] . '\')">
	    <img ';
                if ($mode == "a")
                    echo createComIcon($root_path, 'mans-gr.gif', '0');
                else
                    echo createComIcon($root_path, 'mans-red.gif', '0');
                echo '> ' . ucfirst($row['name_last']) . ', ' . ucwords(ucwords($row['name_first'])) . '</a>
	    <br>';
            }
            echo '
	</font></ul>';
