...
function makefwdpath($path, $dpt, $mo, $yr, $saved) {
    if ($path == 1) {
        ...
        return "nursing-or-dienstplan-checkpoint.php";
}
...
<script language="javascript">
    ...
    function killchild()
    {
        if (window.popselectwin)
            if (!window.popselectwin.closed)
...
if ($saved)
    $sBuffer = createLDImgSrc($root_path, 'close2.gif', '0');
else
    $sBuffer = createLDImgSrc($root_path, 'cancel.gif', '0');
...
for ($i = 1, $n = 0, $wd = $firstday; $i <= $maxdays; $i++, $n++, $wd++) {
    switch ($wd) {
        ...
        case 6: $smarty->assign('sRowClass', 'class="saturday"');
            break;
        case 0: $smarty->assign('sRowClass', 'class="sunday"');
            break;
