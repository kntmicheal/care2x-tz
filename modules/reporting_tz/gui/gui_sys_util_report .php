...
<tbody class="main">
 ...
 <tr valign=top  class="titlebar" >
  <td width="202" bgcolor="#99ccff" >
    &nbsp;&nbsp;<font color="#330066"><?php echo $LDSysUtilsReport; ?></font></td>
...
<!-- END HEAD OF HTML CONTENT -->
					...
					<?php require_once($root_path.$top_dir.'include/inc_gui_timeframe_sys_util_date.php'); ?>
...
$selected_date        =@formatDate2STD($date,$date_format);
...
$query_pharmacy="SELECT b.description,b.price AS unit_price, SUM(b.amount) AS qty
FROM care_tz_billing_archive_elem b
INNER JOIN care_tz_drugsandservices drp ON b.item_number = drp.item_id
WHERE b.date_change BETWEEN $start_timeframe AND $end_timeframe $bill $substore AND (purchasing_class ='drug_list' OR purchasing_class='special_ctc_list' OR purchasing_class='special_others_list')
...
?>
<tr><td><?echo $data['dawa'][$i]['description'];?></td><td><?php echo number_format($data['dawa'][$i]['unit_price'],2);?></td><td><?php echo $data['dawa'][$i]['qty'];?></td><td><?php echo number_format($data['dawa'][$i]['unit_price']*$data['dawa'][$i]['qty'],2);?></td></td></tr>
<?php
}
?>
<tr><td colspan="3"><b>TOTAL COST</b></td><td><b><?php echo number_format($total_cost,2); ?></b></td></tr>
