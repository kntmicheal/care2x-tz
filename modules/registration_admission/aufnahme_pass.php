...
switch ($target) {
    ...
    case 'update':
        $allowedarea = $allow_area['admit_write'];
        $fileforward = 'aufnahme_start.php' . $append . "&pid=$pid&update=$update&origin=patreg_reg&encounter_nr=" . $_SESSION['sess_en'] . '&target=update';
        $lognote = 'Admission ok';
        break;
    case 'search':
...
?>>
    ...
    <P>
        <?php
        switch ($target) {
            case 'entry':$buf = $LDAdmission;
                break;
            case 'search':$buf = $LDAdmTargetSearch;
                break;
