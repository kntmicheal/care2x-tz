...
if ($disablebuttons)
    $smarty->assign('disableButton', true);
...
if ($parent_admit) {
    $smarty->assign('sEncNrPID', $selian_pid);
} else {
    $smarty->assign('sEncNrPID', $selian_pid);
}
...
//        echo 'gui_'.$thisfile;
        ...
        if ($parent_admit && !$is_discharged && $thisfile != 'show_diagnostics_result.php') {
            ...
            if (!empty($externalcall)) {
                $smarty->assign('sPromptLink', '<a href="' . $thisfile . URL_APPEND . '&disablebuttons=' . $disablebuttons . '&pid=' . $_SESSION['sess_pid'] . '&target=' . $target . '&mode=new&help_site&externalcall=' . $externalcall . '"><img ' . createComIcon($root_path, 'createnew_tz.gif', '0') . ' ></a>');
            } else {
                $smarty->assign('sPromptLink', '<a href="' . $thisfile . URL_APPEND . '&disablebuttons=' . $disablebuttons . '&pid=' . $_SESSION['sess_pid'] . '&target=' . $target . '&mode=new&help_site"><img ' . createComIcon($root_path, 'createnew_tz.gif', '0') . ' ></a>');
            }
        } else {
