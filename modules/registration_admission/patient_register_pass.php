...
switch ($target) {
    case 'entry':
        ...
        $fileforward = 'patient_register.php' . $append . '&origin=pass&target=entry';
...
?>>
    ...
    <P>
        ...
        if ($cfg['dhtml']) {
            switch ($target) {
                case 'entry':$buf = $LDPatient . ' :: ' . $LDRegistration;
                    break;
                case 'search':$buf = $LDPatient . ' :: ' . $LDSearch;
                    break;
...
#
		...
		$sNewPatientButton = 'register_green.gif';
                ...
                if ($target == "entry")
                    ...
                    echo'<a href="patient_register_pass.php?sid=' . $sid . '&target=entry&lang=' . $lang . '"><img ' . createLDImgSrc($root_path, $sNewPatientButtonGray, '0') . ' alt="' . $LDNewPerson . '" title="' . $LDNewPerson . '" ';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
                if ($target == "search")
                    echo '<img ' . createLDImgSrc($root_path, 'search_green.gif', '0') . ' alt="' . $LDSearch . '" title="' . $LDSearch . '">';
                else {
                    echo '<a href="patient_register_pass.php?sid=' . $sid . '&target=search&lang=' . $lang . '"><img ' . createLDImgSrc($root_path, 'such-gray.gif', '0') . ' alt="' . $LDSearch . '"  title="' . $LDSearch . '" ';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
                if ($target == "archiv")
                    echo '<img ' . createLDImgSrc($root_path, 'advsearch_green.gif', '0') . '  alt="' . $LDAdvancedSearch . '" title="' . $LDAdvancedSearch . '">';
                else {
                    echo '<a href="patient_register_pass.php?sid=' . $sid . '&target=archiv&lang=' . $lang . '"><img ' . createLDImgSrc($root_path, 'advsearch_gray.gif', '0') . ' alt="' . $LDAdvancedSearch . '"  title="' . $LDAdvancedSearch . '" ';
                    if ($cfg['dhtml'])
                        echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
                    echo '></a>';
                }
