...
$sql_dates="SELECT DATE_FORMAT(encounter_date,'%d.%m.%y') AS admission_date,COUNT(*) AS TOTAL_OPD FROM care_encounter WHERE encounter_class_nr=2 AND DATE_FORMAT
...
$sql_new="SELECT ce.encounter_date, COUNT( * ) AS NEW
      ...
      WHERE encounter_class_nr=2 AND  DATE_FORMAT( ce.encounter_date,  '%Y-%m-%d' ) = DATE_FORMAT( cp.date_reg,  '%Y-%m-%d' ) 
...
$sql_return_under_age="SELECT ce.encounter_date, COUNT( * ) AS RETURN_UNDER_AGE
      FROM care_encounter AS ce INNER JOIN care_person AS cp ON cp.pid = ce.pid 
      WHERE encounter_class_nr=2 AND  AND DATE_FORMAT( ce.encounter_date,  '%Y-%m-%d' ) > DATE_FORMAT( cp.date_reg,  '%Y-%m-%d' ) 
      AND (DATE_FORMAT( NOW( ) , '%Y' ) - DATE_FORMAT( date_birth, '%Y' ) ) <5 AND DATE_FORMAT( ce.encounter_date,  '%Y-%m-%d' ) 
      BETWEEN  '$start' AND  '$end' $department GROUP BY DATE_FORMAT( ce.encounter_date,  '%Y-%m-%d' )";
...
cp ON ce.pid=cp.pid WHERE encounter_class_nr=2 AND DATE_FORMAT(ce.encounter_date,'%Y-%m-%d')=DATE_FORMAT(cp.date_reg,'%Y-%m-%d') AND DATE_FORMAT   
...
cp ON ce.pid=cp.pid WHERE encounter_class_nr=2 AND DATE_FORMAT(ce.encounter_date,'%Y-%m-%d')=DATE_FORMAT(cp.date_reg,'%Y-%m-%d') AND (DATE_FORMAT(ce.encounter_date , '%Y-%m-%d' ) - DATE_FORMAT( date_birth, '%Y-%m-%d' ))<5 AND DATE_FORMAT   
(ce.encounter_date,'%d.%m.%y')='".$dates['dates'][$i]['admission_date']."' $department GROUP BY enrollment_date";
