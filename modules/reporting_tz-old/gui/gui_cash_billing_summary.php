		 ...
		 //echo "Looking for test $TestID by time range: day: ".date("d.m.y", $start_timeframe)." starttime: ".date("d.m.y :: G:i:s",$start_timeframe)." endtime: ".date("d.m.y :: G:i:s", $end_timeframe)."<br>";
		  ...
		  while(list(,$v)=each($res_array))
		  {
			 ...
			 $date_person_reg = $v['REGISTRATION_DATE'];
		  	  ...
		  	  //echo '<tr><td width="79" bgcolor="#ffffaa" widtd="220">'.$v['REGISTRATION_DATE'].'</td>';
		 $sql_dept="SELECT distinct(user_id) as user  FROM care_tz_billing_archive_elem WHERE FROM_UNIXTIME( date_change, '%d.%m.%y' ) ='$date_person_reg'";
			  $db_ptr_dept = $db->Execute($sql_dept);
			  while($db_row_dept=$db_ptr_dept->FetchRow())
			  {
			  $user=$db_row_dept['user'];
			  $sql_bill_amount="SELECT amount,price FROM care_tz_billing_archive_elem WHERE user_id='$user' AND FROM_UNIXTIME( date_change, '%d.%m.%y' ) ='$date_person_reg' AND (balanced_insurance='' OR balanced_insurance='NULL' OR balanced_insurance='0')";
			  $db_ptr_bill_amount = $db->Execute($sql_bill_amount);
			  while($db_row_bill_amount=$db_ptr_bill_amount->FetchRow())
			  {
