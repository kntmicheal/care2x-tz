...
$sql="SELECT  FROM_UNIXTIME(bae.date_change) AS tarehe, bae.description,count(bae.description) AS qty,bae.price
      ...
      BETWEEN $date_from_timestamp AND $date_to_timestamp $ins_id AND drp.purchasing_class='labtest' 
...
$count=count($data['labtest']);
$grand_total=0;
for($i=0; $i<$count; $i++){
$date=date('d.m.Y',strtotime($data['labtest'][$i]['tarehe']));
echo '<tr>';
echo '<td>'.$date.'</td>';
echo '<td>'.$data['labtest'][$i]['description'].'</td>';
echo '<td>'.$data['labtest'][$i]['qty'].'</td>';
echo '<td>'.number_format($data['labtest'][$i]['price'],2).'</td>';
echo '<td>'.number_format($data['labtest'][$i]['qty']*$data['labtest'][$i]['price'],2).'</td>';
echo '</tr>';
$grand_total=$grand_total+($data['labtest'][$i]['qty']*$data['labtest'][$i]['price']);
}
echo '<tr>';
...
<HTML>
    <HEAD>
        ...
        <script language="javascript" >
            ...
            function printOut()
            {
                urlholder = "./laboratory_income.php?printout=TRUE&start=<?php echo $_POST['date_from']; ?>&end=<?php echo $_POST['date_to']; ?>&insurance_type=<?php echo $_POST['paytype'];?>";
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td width="302" bgcolor="#99ccff" > &nbsp;&nbsp;<font color="#330066"><?php echo $LDLabRevReport; ?></font></td>
