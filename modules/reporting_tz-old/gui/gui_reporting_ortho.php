...
$sql="SELECT  FROM_UNIXTIME(bae.date_change) AS tarehe, bae.description,count(bae.description) AS qty,bae.price
      ...
      BETWEEN $date_from_timestamp AND $date_to_timestamp $ins_id AND drp.purchasing_class='ortho_op' 
...
$count=count($data['ortho_op']);
$grand_total=0;
for($i=0; $i<$count; $i++){
$date=date('d.m.Y',strtotime($data['ortho_op'][$i]['tarehe']));
echo '<tr>';
echo '<td>'.$date.'</td>';
echo '<td>'.$data['ortho_op'][$i]['description'].'</td>';
echo '<td>'.$data['ortho_op'][$i]['qty'].'</td>';
echo '<td>'.number_format($data['ortho_op'][$i]['price'],2).'</td>';
echo '<td>'.number_format($data['ortho_op'][$i]['qty']*$data['ortho_op'][$i]['price'],2).'</td>';
echo '</tr>';
$grand_total=$grand_total+($data['ortho_op'][$i]['qty']*$data['ortho_op'][$i]['price']);
}
echo '<tr>';
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td width="302" bgcolor="#99ccff" > &nbsp;&nbsp;<font color="#330066"><?php echo $LDOrthoReport; ?></font></td>
