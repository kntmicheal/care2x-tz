...
$sql="SELECT  FROM_UNIXTIME(bae.date_change) AS tarehe, bae.description,count(bae.description) AS qty,bae.price
      ...
      BETWEEN $date_from_timestamp AND $date_to_timestamp $ins_id AND drp.purchasing_class='xray' 
...
$count=count($data['radiology']);
$grand_total=0;
for($i=0; $i<$count; $i++){
$date=date('d.m.Y',strtotime($data['radiology'][$i]['tarehe']));
echo '<tr>';
echo '<td>'.$date.'</td>';
echo '<td>'.$data['radiology'][$i]['description'].'</td>';
echo '<td>'.$data['radiology'][$i]['qty'].'</td>';
echo '<td>'.number_format($data['radiology'][$i]['price'],2).'</td>';
echo '<td>'.number_format($data['radiology'][$i]['qty']*$data['radiology'][$i]['price'],2).'</td>';
echo '</tr>';
$grand_total=$grand_total+($data['radiology'][$i]['qty']*$data['radiology'][$i]['price']);
}
echo '<tr>';
...
<HTML>
    <HEAD>
        ...
        <script language="javascript" >
            ...
            function printOut()
            {
                urlholder = "./reporting_radiology.php?printout=TRUE&start=<?php echo $_POST['date_from']; ?>&end=<?php echo $_POST['date_to']; ?>&insurance_type=<?php echo $_POST['paytype'];?>";
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td width="302" bgcolor="#99ccff" > &nbsp;&nbsp;<font color="#330066"><?php echo $LDRadiology_rep; ?></font></td>
