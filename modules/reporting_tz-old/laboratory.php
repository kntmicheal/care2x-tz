...
if (empty($_POST['group_id']) && empty($_GET['group_id'])) {
    if ($debug)
        echo "--->Ignition call, setting the requested laboratory group to 1<br>";
    $group_id = "";
} else {
    if ($debug)
        echo "--->getting an post variable about the requested group id<br>";
...
$sql = " INSERT INTO tmp_laboratory_tests(TestID,is_positive,date)
             (SELECT nr, CASE WHEN POSITION('+' IN paramater_name)=1 OR POSITION('check' IN paramater_name)=1 THEN 1 ELSE 0 END,UNIX_TIMESTAMP(care_test_findings_chemlabor_sub.create_time) as date 
...
function NumberOfTests($TestID, $day_as_timestamp) {
    ...
    if ($debug)
        echo "Looking for test $TestID by time range: day: " . date("d.m.y", $day_as_timestamp) . " starttime: " . date("d.m.y :: G:i:s", $start_time) . " endtime: " . date("d.m.y :: G:i:s", $end_time) . "<br>";
    ...
    $row = $db_ptr->FetchRow();
    if ($debug)
        echo "hits:" . $row['number_of_tests'] . "<br><br>";
...
function NumberOfPositiveTests($TestID, $day_as_timestamp) {
    ...
    $row = $db_ptr->FetchRow();
    if ($debug)
        echo "hits:" . $row['number_of_positive_tests'] . "<br><br>";
...
function NumberOfTestsOfMonth($TestID, $start_time) {
    ...
    // getting the day: start_time_frame plus day is what we need:
    if ($debug)
        echo "Looking for test $TestID by time range: day: " . date("d.m.y", $start_time) . " starttime: " . date("d.m.y :: G:i:s", $start_time) . " endtime: " . date("d.m.y :: G:i:s", $end_time) . "<br>";
    $sql = "Select Count(TestID) as number_of_tests FROM tmp_laboratory_tests WHERE TestID=" . $TestID . " AND ( " . $start_time . " <= date AND date <= " . $end_time . " )";
    if ($debug)
        echo $sql . "<br>";
    $db_ptr = $db->Execute($sql);
