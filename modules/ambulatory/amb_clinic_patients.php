    ...
    while ($patient = $opat_obj->FetchRow()) {
        ...
        if ($edit) {
            ...
            if ($dept_nr == 47) {
                $o_arv_patient = new ART_patient($patient['pid']);
                if ($o_arv_patient->is_arv_admitted($patient['pid'])) {
                    $temp_image = "<a href=\"javascript:getARV('" . $patient['pid'] . "','" . $patient['encounter_nr'] . "')\"><img " . createComIcon($root_path, 'powdot.gif', '0', '', TRUE) . " alt=\"inARV\"></a>";
                } else {
                    $temp_image = "<a href=\"javascript:getARV('" . $patient['pid'] . "','" . $patient['encounter_nr'] . "')\"><img " . createComIcon($root_path, 'green_dot.gif', '0', '', TRUE) . " alt=\"not_inARV\"></a>";
                }
            }
