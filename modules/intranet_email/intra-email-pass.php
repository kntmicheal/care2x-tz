...
if (($mode == 'access') && (($username == '') || ($password == '')))
    $onError = $LDErrorIncomplete;
...
if ($onError)
    $smarty->assign('sOnLoadJs', 'onLoad="document.loginform.username.focus();document.loginform.username.select();"');
else if (!$newuser)
    $smarty->assign('sOnLoadJs', 'onLoad="document.loginform.username.focus()"');

if ($regError)
    $smarty->assign('sOnLoadJs', 'onLoad="document.regform.pw1.focus()"');
elseif ($nameError)
    $smarty->assign('sOnLoadJs', 'onLoad="document.regform.addr.focus()"');
elseif ($mode == 'register')
    $smarty->assign('sOnLoadJs', 'onLoad="document.regform.name.focus()"');
