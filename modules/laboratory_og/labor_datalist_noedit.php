...
if ($encounter = &$enc_obj->getBasic4Data($encounter_nr)) {
    ...
    if ($rows = $lab_obj->LastRecordCount()) {
        ...
        if ($nostat)
            ...
            header("location:" . $root_path . "modules/nursing/nursing-station-patientdaten-nolabreport.php?sid=$sid&lang=$lang&edit=$edit&station=$station&pn=$encounter_nr&nodoc=labor&user_origin=$user_origin");
        exit;
    }
}else {
