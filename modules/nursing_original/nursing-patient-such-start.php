...
if ($mode == 'such' || $mode == 'paginate') {
    ...
    if (!$arch)
        $cond.=" AND e.is_discharged IN ('',0) AND p.pid=e.pid ";

    $gbuf = "l.location_nr,UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first,p.date_birth,
...
//		else $cond.=' AND p.pid=e.pid GROUP BY r.location_nr';
    ...
    if ($dblink_ok) {
        $sqlselect = "SELECT r.location_nr AS room_nr, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first,p.date_birth,
					e.encounter_nr, e.encounter_class_nr,e.in_ward,
...
if ($rows) {
    ...
    <td>&nbsp; &nbsp;';
                            if ($result['room_nr'])
                                echo $result['roomprefix'] . ' ' . $result['room_nr'];
                            echo '&nbsp;</td>
