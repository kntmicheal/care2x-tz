...
class Insurance_tz extends Core {
    ...
    function CheckForValidContract($PID, $timestamp = 0, $company_id = 0) {
        ...
        if ($debug)
            echo "CheckForValidContract::GetCompanyFromPID returns: " . $company_id . "<br>";
    ...
    function GetMembersOfContractID($id) {
        ...
        ($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if ($this->debug)
            echo "calling GetMembersOfContractID($id)<br>";

        if (!$id || !is_numeric($id)) {
            if ($this->debug)
                echo "NO PID OR PID IS NOT NUMMERIC<br>";
            return false;
        }
    ...
    function InsertNewInsuranceCompany($dataarray) {
        ...
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if ($dataarray['invoice_flag'])
            $invoice = 1;
        else
            $invoice = 0;
        if ($dataarray['credit_preselection_flag'])
            $credit = 1;
        else
            $credit = 0;
    ...
    function UpdateInsuranceCompany($dataarray) {
        ...
        if ($dataarray['credit_preselection_flag'])
            ...
            $credit = 0;
        if ($dataarray['hide_company_flag'])
            $hide = 1;
        else
            $hide = 0;
    ...
    function UpdateInsuranceType($dataarray) {
        ...
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if (!$dataarray['is_disabled'])
            $dataarray['is_disabled'] = 0;
    ...
    function UpdateContractsArray($dataarray) {
        ...
        if ($debug)
            echo $status['cancel'] . ' ---- ' . $status['enable'];
        return true;
    }
...
//------------------------------------------------------------------------------
    ...
    function ShowMemberBillsOfContract($contract_id, $show_company_info) {
		...
		<table border="0" cellpadding="2" cellspacing="1">
            ...
            if ($this->company['invoice_flag'])
                ...
                echo $LDNo;
            if ($this->company['credit_preselection_flag'])
                echo $LDYes;
            else
                echo $LDNo;
            echo '</td>
    ...
    function CheckContractValidity($company_id, $start_date, $end_date) {
         ...
         * Returns TRUE if this item number still exists in the database
         */
        if (!$company_id || !$start_date || !$end_date || $start_date >= $end_date)
            return false;
        $this->contract_array = $this->GetContractsForCompanyAsArray($company_id);
        while (list($x, $v) = each($this->contract_array)) {
            if (($v['end_date'] > $start_date || $v['start_date'] > $start_date) && $v['cancel_flag'] != 1)
                return false;
        }
        return true;
    }
    ...
    function NewContractForm($company_id) {
        ...
        if ($this->is_company_just_invoiced($company_id)) {
            ...
            echo '
							...
							onKeyUp="setDate(this,\'' . $date_format . '\',\'' . $lang . '\');" name="' . $datename . '">
							<a href="javascript:show_calendar(\'' . $formname . '.' . $datename . '\',\'' . $date_format . '\')">
							<img ' . createComIcon($root_path, 'show-calendar.gif', '0', 'absmiddle') . '></a></td><td>' . $LDPreselectedContractPlan . ':<br>';
            $this->ShowInsuranceTypesDropDown('plan', $v['Contract']['plan']);
            echo '</td></tr>
					</table>';
        }
    }
    ...
    function ShowMemberForms($array) {
        ...
        if (count($contractarray) > 0) {
            if (is_array($array))
                while (list($x, $v) = each($array)) {
                    $result = $person_obj->getAllInfoObject($v['PID']);
                    $person = $result->FetchRow();
                    if ($v['Contract']['gets_company_credit'])
                        $credit = 'Yes';
                    else
                        $credit = 'No';
                    ...
                    if (!is_array($v['Contract']) and $v['Contract'] != -2) {  //New Contract
                        ...
                        if ($v['Contract']['company_id'] != $selected_insurance) { // Is there another valid contract for this person?
                            ...
                            if ($v['Contract']['ceiling_startup_subtraction'])
                                $startupsubstraction = $v['Contract']['ceiling_startup_subtraction'];
                            else
                                $startupsubstraction = 0;
        ...
        while (list($x, $v) = each($allcontracts)) {
    		...
    		</td><td><input type="hidden" name="contract_' . $v['PID'] . '" value="' . $v['id'] . '"><input type="checkbox" name="action_' . $v['PID'] . '" ' . $cancel_string . ' value="cancel"> Cancel contract</td><td align="center">';
            $this->ShowInsuranceTypesDropDown('plan_' . $v['PID'], $v['plan']);
            echo '</td></tr>';
        }
        echo '</table>';
        return true;
    ...
    function ShowAllInsurancesForQuotatuion() {
        ...
        ($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if ($this->debug)
            echo "<br><b>Method class_tz_billing::ShowAllInsurancesForQuotatuion()</b><br>";
    ...
    function is_company_just_invoiced($company_id) {
        ...
        if ($debug)
            echo "return value of is_company_just_invoiced($company_id) is: $this->return_value";
        return $this->return_value;
    }
    ...
    function GetNumberOfMembers($company_id, $year = 0) {
        ...
        if (empty($year))
            $year = date("Y"); // if no year is given, take the current year
