...
class Radio extends Encounter {
    ...
    function hideResultIfExists($enc_nr, $batch_nr) {
        global $_SESSION;
        $this->sql = "UPDATE $this->tb_find_radio SET status='hidden',history=" . $this->ConcatHistory("Hide " . date('Y-m-d H:i:s') . " " . $_SESSION['sess_user_name'] . "\n") . "
    ...
    function GetTestsToDo($id = '') {
        $this->debug = FALSE;
        if ($this->debug)
            echo "calling function GetTestsToDo($id)<br>";
        global $db;
        if (empty($id))
            $cond = '';
        else
    ...
    function searchEncounterRadResults($key = '', $add_opt = '', $limit = FALSE, $len = 30, $so = 0) {
        ...
        if (empty($key))
            return FALSE;
        $this->sql = "SELECT f.encounter_nr,f.findings_date, e.encounter_class_nr, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
        ...
        if (is_numeric($key)) {
            ...
            if ($enc_class)
                $sql.="	AND e.encounter_class_nr=$enc_class";
        }
        # Append the common condition
        $this->sql.=" GROUP BY f.encounter_nr, e.encounter_class_nr, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex  $add_opt";
        //echo $this->sql;
        if ($limit) {
    ...
    function IsMissingLabResult($batch_nr) {
        ...
        if ($debug)
            echo "<br>Records for $batch_nr found in table care_test_request_chemlabor: $num_of_pending_requests<br>";
        ...
        if ($num_of_pending_requests == $num_of_lab_results) {
            if ($debug)
                echo "all results are given! return code is true";
            return 1;
