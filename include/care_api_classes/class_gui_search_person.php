...
class GuiSearchPerson {
    ...
    #Set to true if you want to show the option to select inclusion of the ctc number in universal searches
    var $show_ctc_no = TRUE;
    ...
    function GuiSearchPerson($target = '', $filename = '', $cancelfile = '') {
        ...
        if (!empty($cancelfile))
            ...
            $this->cancelfile = $root_path . $this->cancelfile;
        if (!empty($target)) {
    ...
    function display($skey = '') {
        ...
        $dbf_nodate, $user_origin, $parent_admit, $status, $target, $origin; # $_GET $_POST,
        ...
        if (!defined('SHOW_CTC_NO_CONTROLLER'))
            define('SHOW_CTC_NO_CONTROLLER', $this->show_ctc_no);

        if (SHOW_CTC_NO_CONTROLLER) {
            if (isset($_POST['ctc_no_too'])) {
                if ($_POST['ctc_no_too']) {
                    $ctc_no_too = 1;
                } elseif ($mode == 'paginate' && isset($_GET['ctc_no_too']) && $_GET['ctc_no_too']) {
                    $ctc_no_too = 1;
                }
            } elseif ($mode != 'search') {
                $ctc_no_too = TRUE;
        ...
        if (($this->mode == 'search' || $this->mode == 'paginate') && !empty($searchkey)) {
            ...
            if ($mode == 'paginate') {
                ...
                $ergebnis = $person->SearchSelect($searchkey, $pagen->MaxCount(), $pagen->BlockStartIndex(), $oitem, $odir, $firstname_too, $ctc_no_too);
        ...
        if (defined('SHOW_CTC_NO_CONTROLLER') && SHOW_CTC_NO_CONTROLLER) {
            if (isset($ctc_no_too) && $ctc_no_too)
                $sTemp = 'checked';
            $this->smarty->assign('sCheckBoxCTCNo', '<input type="checkbox" name="ctc_no_too" >');
            $this->smarty->assign('LDIncludeCTCNo', $LDIncludeCTCNo);
        }
        ...
        if ($linecount) {
            ...
            $this->smarty->assign('LDCTCID', $pagen->makeSortLink($LDCTCID, 'ctc_id', $oitem, $odir, $this->targetappend));
            ...
            while ($zeile = $ergebnis->FetchRow()) {
                ...
                if ($zeile['status'] == '' || $zeile['status'] == 'normal') {
                    ...
                    $this->smarty->assign('sCTCNo', $zeile['ctc_id']);

                    switch (strtolower($zeile['sex'])) {
                        case 'f': $this->smarty->assign('sSex', '<img ' . $img_female . '>');
                            break;
                        case 'm': $this->smarty->assign('sSex', '<img ' . $img_male . '>');
                            break;
                        default: $this->smarty->assign('sSex', '&nbsp;');
                            break;
                    }
