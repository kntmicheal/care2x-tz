...
class Encounter extends Notes {
    ...
    function updateServiceClass($type, &$val_array) {
        ...
        if (empty($val_array['sc_' . $type . '_class_nr']))
            return FALSE;
    ...
    function updateCareServiceClass(&$val_array) {
        if (empty($val_array['sc_care_nr']))
            return $this->saveCareServiceClass($val_array);
        else
            return $this->updateServiceClass('care', $val_array);
    }
    ...
    function updateRoomServiceClass(&$val_array, $nr) {
        if (empty($val_array['sc_room_nr']))
            return $this->saveRoomServiceClass($val_array);
        else
            return $this->updateServiceClass('room', $val_array);
    }
    ...
    function updateAttDrServiceClass(&$val_array, $nr) {
        if (empty($val_array['sc_att_dr_nr']))
            return $this->saveAttDrServiceClass($val_array);
        else
            return $this->updateServiceClass('att_dr', $val_array);
    }
    ...
    function loadEncounterData($enc_nr = '') {
        ...
        if (!$this->internResolveEncounterNr($enc_nr))
            return FALSE;
        $this->sql = "SELECT e.*, p.pid, p.selian_pid, p.title, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex,
    ...
    function LastName($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('name_last', $enr, TRUE);
            else
                return FALSE;
        }
    }
    ...
    function FirstName($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('name_first', $enr, TRUE);
            else
                return FALSE;
        }
    }
    ...
    function BirthDate($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('date_birth', $enr, TRUE);
            else
                return FALSE;
        }
    }
    ...
    function PID($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('pid', $enr, TRUE);
            else
                return FALSE;
        }
    }
    ...
    function SelianPID($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('selian_pid', $enr, TRUE);
            else
                return FALSE;
        }
    }
    ...
    function BloodGroup($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('blood_group', $enr, TRUE);
            else
                return FALSE;
        }
    }
    ...
    function EncounterClass($enr = 0) {
        ...
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('encounter_class_nr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function FinancialClass($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('financial_class', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RefererDiagnosis($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('referrer_diagnosis', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RefererRecomTherapy($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('referrer_recom_therapy', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RefererNotes($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('referrer_notes', $enr);
            else
                return FALSE;
        }
    }
    ...
    function Referer($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('referrer_dr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RefererDept($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('referrer_dept', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RefererInstitution($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('referrer_institution', $enr);
            else
                return FALSE;
        }
    }
    ...
    function InsuranceNr($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('insurance_nr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function InsuranceFirmID($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('insurance_firm_id', $enr);
            else
                return FALSE;
        }
    }
    ...
    function CurrentWardNr($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('current_ward_nr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function CurrentRoomNr($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('current_room_nr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function CurrentDeptNr($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('current_dept_nr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function CurrentFirmNr($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('current_firm_nr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function CurrentAttDrNr($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('current_att_dr_nr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function In_Ward($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('in_ward', $enr);
            else
                return FALSE;
        }
    }
    ...
    function In_Dept($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('in_dept', $enr);
            else
                return FALSE;
        }
    }
    ...
    function EncounterStatus($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('encounter_status', $enr);
            else
                return FALSE;
        }
    }
    ...
    function EncounterType($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('encounter_type', $enr);
            else
                return FALSE;
        }
    }
    ...
    function ConsultingDr($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('consulting_dr', $enr);
            else
                return FALSE;
        }
    }
    ...
    function FollowUpDate($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('followup_date', $enr);
            else
                return FALSE;
        }
    }
    ...
    function FollowUpResponsibility($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('followup_responsibility', $enr);
            else
                return FALSE;
        }
    }
    ...
    function PostEncounterNotes($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('post_encounter_notes', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RecordStatus($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('status', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RecordHistory($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('history', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RecordModifierID($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('modify_id', $enr);
            else
                return FALSE;
        }
    }
    ...
    function RecordCreatorID($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('create_id', $enr);
            else
                return FALSE;
        }
    }
    ...
    function PhotoFilename($enr = 0) {
        if ($this->is_loaded) {
            ...
            if ($enr)
                return $this->getValue('photo_filename', $enr, TRUE);
            else
                return FALSE;
        }
    }
    ...
    function _searchAdmissionBasicInfo($key, $enc_class = 0, $add_opt = '', $limit = FALSE, $len = 30, $so = 0) {
        ...
        $db->debug = FALSE;
        if ($db->debug)
            echo "_searchAdmissionBasicInfo is called with enc_class=" . $enc_class . "<br>";
        if ($enc_class == 2) {
            ...
            $this->sql = "SELECT e.encounter_nr, e.encounter_class_nr, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.selian_pid, p.sex,p.blood_group
					FROM $this->tb_enc AS e LEFT JOIN $this->tb_person AS p ON e.pid=p.pid";
        }
        if (is_numeric($key)) {
            if ($db->debug)
...
//		global $_SESSION;
    ...
    function _discharge($enr, $loc_types, $d_type_nr, $date = '', $time = '', $transferToWard) {
        global $dbf_nodate, $dbtype; // $_SESSION;
        if (empty($date))
            $date = date('Y-m-d');
        if (empty($time))
            $time = date('H:i:s');
    ...
    function getBasic4Data($enc_nr) {
        ...
        if (!$this->internResolveEncounterNr($enc_nr))
            return FALSE;
        $this->sql = "SELECT UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
							FROM $this->tb_enc AS e,
									 $this->tb_person AS p
							WHERE e.encounter_nr=$this->enc_nr
								AND e.pid=p.pid";
...
//	    global $_SESSION;
        if (!$appt_nr || !$this->internResolveEncounterNr($enc_nr))
            return FALSE;
    ...
    function OutPatientsBasic($sort = "e.encounter_class_nr", $dept_nr = 0) {
        ...
        if ($dept_nr)
            $cond = "e.current_dept_nr=$dept_nr AND";
        else
            $cond = '';
        //$cond='';
    ...
    function createWaitingOutpatientList($dept_nr = 0) {
        ...
        $this->sql = "SELECT e.encounter_nr, e.encounter_class_nr, e.current_dept_nr,
									p.pid, CONCAT(p.name_first,' ', p.name_2) AS name_first, CONCAT(UPPER(p.name_last),', ') as name_last,
                                                                        p.date_birth, p.sex,p.selian_pid, d.nr AS dept_nr, d.name_short, d.LD_var AS \"dept_LDvar\"
				FROM $this->tb_enc AS e
					LEFT JOIN $this->tb_person AS p ON e.pid=p.pid
    ...
    function getValue($item, $enr = '', $person = FALSE) {
        ...
        if ($this->is_loaded) {
            if (isset($this->encounter[$item]))
                return $this->encounter[$item];
            else
                return false;
        } else {
