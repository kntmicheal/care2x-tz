...
#        global $_POST;
    ...
    function CityTownName($code_nr = '') {
        global $db;
        if (!$this->is_preloaded)
            $this->sql = "SELECT name FROM $this->tb_citytown WHERE nr=$code_nr";
        else
            $this->sql = "SELECT name FROM $this->tb_citytown WHERE nr=" . $this->CityTownCode();
    ...
    function Persons($searchkey = '', $order_item = 'name_last', $order_dir = 'ASC') {
        ...
        /*
          $this->sql="SELECT pid, CONCAT(UPPER(name_last),', ') as name_last, CONCAT(name_first,' ', name_2) AS name_first, date_birth, sex FROM $this->tb_person WHERE status NOT IN ($this->dead_stat) ";
          if(!empty($searchkey)){
    ...
    function SearchSelect($searchkey = '', $maxcount = 100, $offset = 0, $oitem = 'name_last', $odir = 'ASC', $fname = FALSE, $ctc_no = FALSE) {
        ...
        if (is_numeric($suchwort)) {
            ...
            if (empty($oitem))
                $oitem = 'pid';
            if (empty($odir))
                $odir = 'DESC';# default, latest pid at top
        ...
        if (defined('SHOW_CTC_NO_CONTROLLER') && SHOW_CTC_NO_CONTROLLER) {       //Search by CTC ID if checked
            if ($ctc_no) {
                $this->sql = "SELECT a.ctc_id, p.pid, p.selian_pid, p.name_2, p.name_last, name_first, p.date_birth, p.addr_zip, p.sex, p.death_date, p.status FROM care_person p, care_tz_arv_registration a "
                        . ' WHERE a.pid = p.pid '
                        . " AND status NOT IN ($this->dead_stat) "
                        . " AND a.ctc_id LIKE " . $searchkey . " $sql3";
//                echo $searchkey;
            } else {
    ...
    function postgre_PIDbyOID($oid = 0) {
        if (!$oid)
            ...
            return $this->postgre_Insert_ID($this->tb_person, 'pid', $oid);
    }
    ...
    function PIDbyData($data, $deadtoo = FALSE) {
        ...
        if (!$deadtoo)
            $this->sql.=" AND death_date='$dbf_nodate'";
