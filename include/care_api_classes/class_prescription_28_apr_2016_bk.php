...
class Prescription extends Core {
	...
	*/
	...
	function getDrugList($class) {
		
		global $db;
		global $is_transmit_to_weberp_enable,$search_query,$quick_search;
		echo $class;
		echo $search_query;
		$weberp_cat = '';
		if($is_enabled){			
	     ...
	     if(isset($search_query)|| !empty($search_query)){
			 $quick_search='AND description like '."'%".$search_query."%'";
		...
		$this->sql='SELECT  item_description as description, item_id as drug_id, partcode as stockid, not_in_use FROM '.
			$this->tb_drugsandservices.' WHERE purchasing_class = "'.$class.'" '.$query.' "'.$quick_search.'" ORDER BY description';
