...
//		echo $this->sql;
    ...
    function StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $insurance, $pricelist_no) {
		...
		INSERT INTO $this->tbl_bill_elements " .
                ...
                "		User_Id)" .
                "
		VALUES (" . $bill_number . ", '" . time() . "', 0, 1, 0, 0, 0, '" . $row['total_dosage'] . "', '" . $row['total_dosage'] . "', " . $row['times_per_day'] . "," . $row['days'] . ", " . $price . ", '" . $row['article'] . "', '" . $row['notes'] . "', " . $row['article_item_number'] . ", '" . $insurance . "', '" . $contract['id'] . "', '" . $row['nr'] . "','" . $row['sub_store'] . "','" . $user_id . "')";
    ...
    function StoreAdvancePaymentToBill($pid, $bill_number, $insurance, $advance_payment, $notes, $description) {
        ...
        if ($this->CheckUserID($user_id)) {
			...
			INSERT INTO $this->tbl_bill_elements  " .
                    ...
                    "		User_Id)" .
                    "
			VALUES (" . $bill_number . ", '" . time() . "', 0, 0, 0, 0, 1, 1, 1, 0, 0, " . $advance_payment . ", '" . $description . "', '" . $notes . "',0 , '" . $insurance . "', 0, 0,'" . $user_id . "')";
...
//------------------------------------------------------------------
//Function to check if the user_id is not empty
...
//---------------------------------------------------------------------------------
    ...
    function DisplayBillHeadline($bill_nr, $batch_nr) {
        ...
        if ($enc_obj->EncounterExists($enc_number)) {
            ...
            $insurance_name=$this->insur_name($file_no);
            ...
            echo '
				...
				<tr>
  				...
  				<tr>
  					...
  					<tr>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="3">Health Fund:</font></span></span></td>
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="3">' . $insurance_name . '</font></span></span></b></td>
  					</tr>';
...
<tr><td align="right"><img src="../../' . $bill_logo . '" border=0 align="absmiddle" alt=""></td></tr></table>';
    ...
    function DisplayLaboratoryBill($bill_nr, $edit_fields) {
        ...
        while ($bill_elems_row = $billelems->FetchRow()) {
            ...
            //description
            echo '<tr><td><b>' . $desc . '</b></td></tr>';
            echo '<tr><td> ' . $price . ' x' . '' . $bill_elems_row['amount'] . ' =' . '</td><td>' . number_format(($price * $bill_elems_row
                    ['amount']), 0, '.', ',') . '</td></tr>';
            ...
            /*
              ...
              echo '<td><b>'.number_format(($price*$bill_elems_row['amount'])-$bill_elems_row
              ['balanced_insurance'],0,'.',',').'</b></td>';
    ...
    function DisplayPrescriptionBill($bill_nr, $edit_fields) {
        ...
        if ($sum > 0)
            echo '<tr><td><b>PHARMACY=</b></td><td>' . number_format(($sum), 0, '.', ',') . '</td></tr>';
...
<tbody class="main">
    ...
    function ArchiveBill($bill_number) {
        ...
        $this->sql = "INSERT INTO care_tz_billing_archive_elem
                      (`nr`,`date_change`, `is_labtest`,`is_radio_test`, `is_medicine`, `is_comment`, `is_paid`, `amount`, `days`, `times_per_day`, `price`, `balanced_insurance`, `insurance_id`, `description`, `item_number`, `prescriptions_nr`,`sub_store`, `User_Id`)
                  SELECT `nr`,`date_change`, `is_labtest`,`is_radio_test`, `is_medicine`, `is_comment`, 1, `amount`, `days`, `times_per_day`, `price`, `balanced_insurance`, `insurance_id`, `description`, `item_number`, `prescriptions_nr`,`sub_store`,`User_Id` FROM care_tz_billing_elem WHERE `nr`=" . $bill_number;
        $this->result = $db->Execute($this->sql);
    ...
    function TransferBillItemsWithAdvance($bill_number) {
        ...
        $debug = FALSE;
        if ($debug)
            echo "<b>class_tz_billing::TransferBillItemsWithAdvance($bill_number)</b><br>";
        ($debug) ? $db->debug = FALSE : $db->debug = FALSE;
        ...
        $this->sql = "INSERT INTO care_tz_billing_elem_advance
                  ...
                  SELECT `nr`,`date_change`, `is_labtest`, `is_medicine`, `is_comment`,1, `amount`, `days`, `times_per_day`, `price`, `balanced_insurance`, `insurance_id`, `description`, `item_number`, `prescriptions_nr`,`User_Id` FROM care_tz_billing_elem WHERE `description`!='Refund' AND  `nr`=" . $bill_number;
...
//		echo '<tr><td colspan="8">'.$LDNothingtodo.'</td></tr>';
 ...
 function insur_name($file_number){
	 global $db;
     $this->debug = false;
    ($this->debug) ? $db->debug = FALSE : $db->debug = FALSE;
     $sql_insur="SELECT insurance_ID FROM care_person WHERE selian_pid=".$file_number;
     $insur_result=$db->Execute($sql_insur);
     if($rows_insur=$insur_result->FetchRow()){
		 $insur_id=$rows_insur['insurance_ID'];
		 $sql_insur_name="SELECT name FROM care_tz_company WHERE id=".$insur_id;
		 $insur_name_result=$db->Execute($sql_insur_name);
		 if($ins_name_rows=$insur_name_result->FetchRow()){
			 $insur_name=$ins_name_rows['name'];
			 }
		 }
