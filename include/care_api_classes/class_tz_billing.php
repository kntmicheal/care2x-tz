...
class Bill extends Encounter {
    ...
    function getPrice($item_id, $pricelist_no) {
        ...
        $this->debug = FALSE;
        if ($this->debug)
            echo "<b>getPrice($item_id, $pricelist_no=1)</b>";
...
//------------------------------------------------------------------------------
    ...
    function DisplayArchivedBillAmount($bill_nr) {
        ...
        ;
        if ($this->debug)
            echo "<b>class_tz_billing:: DisplayArchivedBillAmount($bill_nr)</b><br>";
        $total_bill = 0;

        ($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;

        $this->sql = "SELECT * FROM care_tz_billing_archive_elem WHERE is_deposit_item!=1 AND nr = " . $bill_nr;
...
//		echo $this->sql;
    ...
    function StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $insurance, $pricelist_no) {
        ...
        $this->debug = FALSE;
        if ($this->debug)
            echo "<b>class_tz_billing::StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $price, $dosage, $notes, $insurance, $timesPerDay, $days)</b><br>";
        if ($this->debug)
        ...
        $this->sql = "SELECT $this->tbl_prescriptions.*,ce.current_dept_nr,ce.current_ward_nr,ce.encounter_class_nr FROM $this->tbl_prescriptions INNER JOIN care_encounter ce ON ce.encounter_nr=$this->tbl_prescriptions.encounter_nr WHERE nr = " . $prescriptions_nr;
		...
		INSERT INTO $this->tbl_bill_elements " .
                ...
                "		User_Id)" .
                "
		VALUES (" . $bill_number . ", '" . time() . "', 0, 1, 0, 0, 0, '" . $row['total_dosage'] . "', '" . $row['total_dosage'] . "', " . $row['times_per_day'] . "," . $row['days'] . ", " . $price . ", '" . $row['article'] . "', '" . $row['notes'] . "', " . $row['article_item_number'] . ", '" . $insurance . "', '" . $contract['id'] . "', '" . $row['nr'] . "','" . $row['sub_store'] . "','" . $row['current_dept_nr'] . "','" . $row['current_ward_nr'] . "','" . $row['encounter_class_nr'] . "','" . $user_id . "')";
    ...
    function StoreLaboratoryItemToBill($pid, $labtest_nr, $batch_nr, $bill_number, $insurance, $pricelist_no) {
		...
		care_tz_drugsandservices.item_description,care_encounter.current_dept_nr,care_encounter.current_ward_nr,care_encounter.encounter_class_nr
        ...
        while ($row = $result->FetchRow()) {
			...
			INSERT INTO $this->tbl_bill_elements  " .
                    ...
                    "		User_Id)" .
                    "
			VALUES (" . $bill_number . ", '" . time() . "', 1, 0, 0, 0, 1, 1, 1, 0, 0, " . $price . ", '" . $row['item_description'] . "', 0, " . $row['p_item_id'] . ", '" . $insurance . "', '" . $contract['id'] . "', '" . $row['prescrip_nr'] . "', '" . $row['current_dept_nr'] . "', '" . $row['current_ward_nr'] . "', '" . $row['encounter_class_nr'] . "','" . $user_id . "')";
    ...
    function StoreRadiologyItemToBill($pid, $batch_nr, $bill_number, $insurance, $pricelist_no) {
		...
		care_tz_drugsandservices.item_id,care_tz_drugsandservices.item_description,care_encounter.current_dept_nr,care_encounter.current_ward_nr,care_encounter.encounter_class_nr
 ...
 '" . $row['item_description'] . "', " . $row['item_id'] . ", '" . $insurance . "', '" . $contract['id'] . "', " . $row['batch_nr'] . "," . $row['current_dept_nr'] . ", " . $row['current_ward_nr'] . ", " . $row['encounter_class_nr'] . ",'" . $user_id . "')";
    ...
    function StoreAdvancePaymentToBill($pid, $bill_number, $insurance, $advance_payment, $notes, $description) {
        global $db, $root_path;
        global $user_id, $sid, $local_user;
        $this->debug = false;
        ($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if ($this->debug)
            echo "<b>class_tz_billing::StoreAdvancePaymentToBill(pid: $pid, bill_number: $bill_number, insurance: $insurance,advance_payment:$advance_payment)</b><br>";

        $user_id = $_SESSION['sess_user_name'];

        $sql_InOut_Patient = "SELECT b.encounter_nr,ce.current_dept_nr,ce.current_ward_nr,ce.encounter_class_nr
                    FROM care_tz_billing b 
                    INNER JOIN care_encounter ce
                    ON ce.encounter_nr=b.encounter_nr
                    WHERE b.nr=" . $bill_number;
        $result = $db->Execute($sql_InOut_Patient);
        if ($row = $result->FetchRow()) {
            $current_dept_nr = $row['current_dept_nr'];
            $current_ward_nr = $row['current_ward_nr'];
            $encounter_class_nr = $row['encounter_class_nr'];
        }
        ...
        $this->sql = "
			INSERT INTO $this->tbl_bill_elements  " .
                ...
                "		User_Id)" .
                "
			VALUES (" . $bill_number . ", '" . time() . "', 0, 0, 0, 0, 1, 1, 1, 0, 0, " . $advance_payment . ", '" . $description . "', '" . $notes . "',0 , '" . $insurance . "', 0, 0,'" . $current_dept_nr . "','" . $current_ward_nr . "','" . $encounter_class_nr . "','" . $user_id . "')";
        //echo $this->sql;
        if ($this->debug)
            echo $this->sql;
        $db->Execute($this->sql);
    ...
    function GetAdvanceTotalAmount($encounter_nr) {
        ...
        ;
        if ($this->debug)
            echo "<b>class_tz_billing:: GetTotalAdvanceAmount($encounter_nr)</b><br>";
        $total_advance = 0;
	...
	WHERE (description='Topup' OR description='Refund') AND care_tz_billing_archive.encounter_nr = " . $encounter_nr;
        $this->result = $db->Execute($this->sql);
        if ($rows = $this->result->FetchRow()) {
            $total_advance = 0;
        }
...
//------------------------------------------------------------------------------
//this function will check if the advance has refund or topup
    function CheckFinalBill($encounter_nr) {
        global $db, $root_path;
        $this->sql = "SELECT * FROM care_tz_billing_archive_elem
			INNER JOIN care_tz_billing_archive
			ON
...
//------------------------------------------------------------------------------
    function TotalDeposit($encounter_nr) {
...
//------------------------------------------------------------------------------
    ...
    function GetAllAdvancesForEncounter($encounter_nr) {
        ...
        ;
        if ($this->debug)
            echo "<b>class_tz_billing:: GetAllAdvancesForEncounter($encounter_nr)</b><br>";
        $total_advance = 0;
...
// end of function GetNameOfDrug($item_number)
    ...
    function GetElemsOfArchivedBill($nr, $what_kind_of) {
        ...
        $this->sql = "SELECT $this->tbl_bill_archive_elements.*, care_tz_drugsandservices.purchasing_class FROM $this->tbl_bill_archive_elements
                                INNER JOIN care_tz_drugsandservices ON $this->tbl_bill_archive_elements.item_number=care_tz_drugsandservices.item_id
                                         WHERE is_deposit_item!=1 AND nr = " . $nr . " $sql_where
                                         ORDER BY date_change ASC";
        //echo $this->sql;                          
    ...
    function GetElemsOfBill($nr, $what_kind_of) {
        ...
        if ($what_kind_of == "drug_list" || $what_kind_of == "prescriptions") {
            $sql_where = "AND (purchasing_class LIKE 'drug_list%' OR purchasing_class='supplies')";
        } else
        ...
        if ($what_kind_of == "other") {
            $sql_where = "AND (purchasing_class NOT LIKE 'drug_list%'
				AND  purchasing_class!='supplies'
				AND  purchasing_class!='labtest'
				AND  purchasing_class!='xray'
				AND  purchasing_class!='service')";
        ...
        $this->sql = "SELECT $this->tbl_bill_elements.*, care_tz_drugsandservices.purchasing_class FROM $this->tbl_bill_elements
				INNER JOIN care_tz_drugsandservices ON $this->tbl_bill_elements.item_number=care_tz_drugsandservices.item_id
					 WHERE $this->tbl_bill_elements.is_deposit_item!=1  AND nr = " . $nr . " $sql_where
					 ORDER BY date_change ASC";
//		echo $this->sql;
        return $db->Execute($this->sql);
    }
...
//------------------------------------------------------------------------------
    function GetRefundBillElem($nr) {
        global $db;
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;

        $this->sql = "SELECT * from $this->tbl_bill_elements
	WHERE nr = " . $nr . " AND description ='Refund'
					 ORDER BY date_change ASC";
...
//------------------------------------------------------------------------------
    function GetTopupBillElem($nr) {
        global $db;
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;

        $this->sql = "SELECT * from $this->tbl_bill_elements
	WHERE nr = " . $nr . " AND description ='Topup'
					 ORDER BY date_change ASC";
...
//----------------------------------------------------------------------------
    function GetArchivedRefundBillElem($nr) {
        global $db;
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
...
//---------------------------------------------------------------------------------
    function GetArchivedTopupBillElem($nr) {
        global $db;
        $debug = false;
        ($debug) ? $db->debug = TRUE : $db->debug = FALSE;
...
//---------------------------------------------------------------------------------
    ...
    function DisplayBillHeadline($bill_nr, $batch_nr) {
        ...
        if ($enc_obj->EncounterExists($enc_number)) {
            ...
            $insname = $enc_obj->GetHealthFundName($file_no);
...
<tr><td align="right"><img src="../../' . $bill_logo . '" border=0 align="absmiddle" alt=""></td></tr></table>';
            ...
            echo '
				...
				<tr>
  					...
  					<tr>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="3">PATIENT TYPE.:</font></span></span></td>
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="3">' . $insname . '</font></span></span></b></td>
  					</tr>

  				<tr>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="3">Names:</font></span></span></td>
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="3">' . $enc_obj->LastName($enc_number) . ', ' . ucwords($enc_obj->FirstName($enc_number)) . '</font></span></span></b></td>
  					</tr>';
    ...
    function DisplayArchivedBillHeadline($bill_nr, $batch_nr) {
        ...
        if ($enc_obj->EncounterExists($enc_number)) {
            ...
            $file_no = $enc_obj->SelianPID($enc_number);
            $insname = $enc_obj->GetHealthFundNameArchived($bill_nr);
  	...
  	<table width="100%" border="0" class="table_content">
        ...
        while ($bill_elems_row = $billelems->FetchRow()) {
            ...
            if ($bill_elems_row['is_radio_test'] == 1) {
                ...
                if (empty($bill_elems_row['price']))
                    $bill_elems_row['price'] = "0,00";
            }
            $part_sum = ($bill_elems_row['price'] * $bill_elems_row['amount']) - $bill_elems_row['balanced_insurance'];
            $sum += $part_sum;
            echo '
      				<tr>
      				  <td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1">' . $pos_nr . '</font></span></span></td>
        				<td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1">' . $bill_elems_row['description'] . '</font></span></span></td>
        				<td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1">' . $bill_elems_row['price'] . '</font></span></span></td>
        				<td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1">' . $bill_elems_row['amount'] . '</font></span></span></td>' .
            '<td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1">';
            if ($bill_elems_row['balanced_insurance'] > 0)
                echo number_format($bill_elems_row['balanced_insurance'], 2, '.', ',');
...
//------------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    function DisplayRefundBillItem($bill_nr, $edit_fields) {
        ...
        $billelems = $this->GetRefundBillElem($bill_nr);
    ...
    //-----------------------------------------------------------------------------
    function DisplayTopupBillItem($bill_nr, $edit_fields) {
        ...
        $billelems = $this->GetTopupBillElem($bill_nr);
...
//------------------------------------------------------------------------------------------
    function DisplayArchivedRefundBill($bill_nr, $edit_fields) {
  	...
  	<table width="100%" border="0" class="table_content">
        ...
        $billelems = $this->GetArchivedRefundBillElem($bill_nr);
        //echo "edit fields is set to:".$edit_fields;
        echo '
      			<table width="100%" height="100%" class="table_content">
	      			<tr>
    ...
    function DisplayArchivedTopupBill($bill_nr, $edit_fields) {
  	...
  	<table width="100%" border="0" class="table_content">
        ...
        $billelems = $this->GetArchivedTopupBillElem($bill_nr);
        //echo "edit fields is set to:".$edit_fields;
        echo '
      			<table width="100%" height="100%" class="table_content">
	      			<tr>
...
<tbody class="main">
    ...
    function DisplayArchivedBills($batch_nr, $specific_bill, $edit_fields) {
        ...
        echo '
	  	<table width="200"  border="" align="center" class="table_content">';
        ...
        if ($billnumbers)
            while ($bills = $billnumbers->FetchRow()) {
                ...
                $this->DisplayArchivedBillHeadline($bills['nr'], $batch_nr);
                echo '
  						</font></span></span></td>
  					</tr>';
                $sum_to_pay = 0;
                ...
                $billelems = $this->GetArchivedRefundBillElem($bills['nr']);
                if ($bill_elems_row = $billelems->FetchRow()) {
                    echo '
	  	  			<tr>
	  	  				<td valign="top"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">';
                    $total_refund = $this->DisplayArchivedRefundBill($bills['nr'], $edit_fields);
                    echo '
		  	      		</font></span></span></td>
		  	      	</tr>';
                }

                $billelems = $this->GetArchivedTopupBillElem($bills['nr']);
                if ($bill_elems_row = $billelems->FetchRow()) {
                    echo '
	  	  			<tr>
	  	  				<td valign="top"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">';
                    $total_topup = $this->DisplayArchivedTopupBill($bills['nr'], $edit_fields);
                    echo '
		  	      		</font></span></span></td>
		  	      	</tr>';
                }
                ...
                $sum_to_pay += $total_refund;
                $sum_to_pay += $total_topup;
    ...
    function DisplayBills($batch_nr, $specific_bill, $edit_fields) {
        ...
        if ($billnumbers) {
            ...
            while ($bills = $billnumbers->FetchRow()) {
                ...
                $matchingContract = $insurance_tz->GetContractMemberFromTimestamp($batch_nr, $bill_timestamp);
                $matchingBills = $bill_obj->GetBillCostSummaryInTimeframe($batch_nr, $matchingContract['start_date'], $bill_timestamp);
                $ceiling = $matchingContract['Member']['ceiling'] - $matchingContract['Member']
                        ['ceiling_startup_subtraction'];
                $used_budget = array_sum($matchingBills);
                $insurancebudget = $ceiling - $used_budget;
                $this->debug = FALSE;
                ...
                $billelems = $this->GetRefundBillElem($bills['nr']);
                if ($billelems) {
					...
					<tr>
						<td valign="top"><span class=SpellE><span style="font-family;&quot;20 cpi&quot;"<font size=-1>';
                    $total_refund = $this->DisplayRefundBillItem($bills['nr'], $edit_fields, $printout);
                    echo '
						</font></span></span></td>
					</tr>';
                }
                $billelems = $this->GetTopupBillElem($bills['nr']);
                if ($billelems) {
					...
					<tr>
						<td valign="top"><span class=SpellE><span style="font-family;&quot;20 cpi&quot;"<font size=-1>';
                    $total_refund = $this->DisplayTopupBillItem($bills['nr'], $edit_fields, $printout);
                    echo '
						</font></span></span></td>
					</tr>';
                }
                ...
                echo ($printout) ? '<table  width="100%" border="0">' : '<table width="600" border="0" class="table_content">';
                if ($total_laboratory || $total_prescription || $total_radiology || $total_services || $total_advance)
                    echo '<tr>
                ...
                if ($total_laboratory)
                    ...
                    '<tr>
											...
											<td  style="text-align:left;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="3">
												' . number_format(($total_laboratory), 2, '.', ',') . '
											</font></span></span></td>
										</tr>';
                if ($total_radiology)
                ...
                '<tr>
											...
											<td style="text-align:left;"><font color="#FF0000"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="4">
    ...
    function ArchiveBill($bill_number) {
        ...
        $this->sql = "INSERT INTO care_tz_billing_archive_elem
                      (`nr`,`date_change`, `is_labtest`,`is_radio_test`, `is_medicine`, `is_comment`, `is_paid`, `amount`, `days`, `times_per_day`, `price`, `balanced_insurance`, `insurance_id`, `description`, `item_number`, `prescriptions_nr`,`sub_store`,`is_deposit_item`, `User_Id`,current_dept_nr,current_ward_nr,encounter_class_nr)
                  SELECT `nr`,`date_change`, `is_labtest`,`is_radio_test`, `is_medicine`, `is_comment`, 1, `amount`, `days`, `times_per_day`, `price`, `balanced_insurance`, `insurance_id`, `description`, `item_number`, `prescriptions_nr`,`sub_store`,`is_deposit_item`,`User_Id`,current_dept_nr,current_ward_nr,encounter_class_nr FROM care_tz_billing_elem WHERE `nr`=" . $bill_number;
        $this->result = $db->Execute($this->sql);
    ...
    function DeleteBillAdvanceItemsFromPendingList($bill_number) {
        ...
        $debug = FALSE;
        if ($debug)
            echo "<b>class_tz_billing::DeleteBillAdvanceItemsFromPendingList($bill_number)</b><br>";
        ($debug) ? $db->debug = FALSE : $db->debug = FALSE;
        $this->sql = "DELETE FROM care_tz_billing_elem WHERE `description`!='Advance' AND `description`!='Refund' AND `description`!='Topup' AND `nr`=" . $bill_number;
        $db->Execute($this->sql);
        return TRUE;
    }
    ...
    //------------------------------------------------------------------------------
    function FlagDepositItems($bill_number) {
        global $db;
        global $user_id, $sid, $local_user;
        $debug = FALSE;
        if ($debug)
            echo "<b>class_tz_billing::FlagDepositItems($bill_number)</b><br>";
        ($debug) ? $db->debug = FALSE : $db->debug = FALSE;


        $this->sql = "UPDATE care_tz_billing_elem SET is_deposit_item=1 WHERE description NOT IN('Refund','Topup') AND  `nr`=" . $bill_number;
    ...
    function CreateTmpTableOfElementsToQuote($encounter_nr, $in_outpatient) {
        ...
        ($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if ($this->debug)
            echo "<br><b>Method class_tz_billing::CreateTmpTableOfElementsToQuote()</b><br>";
    ...
    function GetNewQuotationOverview() {
        ...
        ($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;
        if ($this->debug)
            echo "<br><b>Method class_tz_billing::GetNewQuotationOverview()</b><br>";


        $this->sql = "SELECT
						count(*) as anzahl, encounter_nr,current_dept_nr, current_ward_nr, pid, selian_pid, CONCAT(name_first,' ', name_2) AS name_first, UPPER(name_last) as name_last,
date_birth, encounter_date , encounter_class_nr, insurance_ID
    ...
    function GetNewQuotations($encounter_nr, $in_outpatients) {
        ...
        $db->debug = FALSE;
        $this->sql = "SELECT
						count(*) as anzahl, encounter_nr,pid, selian_pid, UPPER(name_last) as name_last, CONCAT(name_first,'  ', name_2) AS name_first, date_birth, encounter_date
					FROM quotation_temporary
...
//------------------------------------------------------------------------------
    ...
    function UpdateBillNumberNewLaboratory($nr, $bill_number) {
        ...
        ;
        if ($debug)
            echo "<b>class_tz_billing::UpdateNewLaboratory($nr)</b><br>";
        ($debug) ? $db->debug = FALSE : $db->debug = FALSE;
        if (!$nr)
            return false;
        $this->sql = "UPDATE $this->tbl_lab_requests_sub SET
...
//------------------------------------------------------------------------------
    ...
    function UpdateBillNumberNewRadiology($nr, $bill_number) {
        ...
        ;
        if ($debug)
            echo "<b>class_tz_billing::UpdateNewRadiology($nr)</b><br>";
        ($debug) ? $db->debug = FALSE : $db->debug = FALSE;
        if (!$nr)
            return false;
    ...
    function ShowNewQuotations_Prescriptions($in_outpatient, $sid) {
        ...
        if ($result) {
          ...
          <tr>
					  ...
					  <td ' . $BGCOLOR . ' class="td_content"><div align="center"><input type="hidden" name="namelast" value="' . $row['name_last'] . '"><input type="hidden" name="patient" value="' . $_REQUEST['patient'] . '"><input type="hidden" name="namefirst" value="' . ucwords($row['name_first']) . '"><input type="hidden" name="countpres" value="' . $row['anzahl'] . '"><input type="hidden" name="countlab" value="' . $row['anzahl_lab'] . '"><input type="hidden" value="' . $row['encounter_nr'] . '" name="encounter_nr"><input type="hidden" value="' . $row['pid'] . '" name="pid"><input type="submit" value=">>"></div></td>
					  </form>
					</tr>';
                $alreadyshown[$row['encounter_nr']] = $row['encounter_nr'];
            }
        ...
        if ($result) {
          ...
          <tr>
          	<form method="POST" action="billing_tz_quotation_create.php">
					  ...
					  <td ' . $BGCOLOR . '><div align="center">' . $row['name_last'] . ', ' . ucwords($row['name_first']) . '</div></td>
        ...
        if ($result) {
            ...
            while ($row = $result->FetchRow()) {
                ...
                switch ($_REQUEST['unit_price']) {
                    ...
                    case 5: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_4 . '">';
                        $unitPrice = $unit_price_4;
                        break;
                    case 6: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_5 . '">';
                        $unitPrice = $unit_price_5;
                        break;
                    case 7: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_6 . '">';
        ...
        if ($result) {
            ...
            while ($row = $result->FetchRow()) {
                ...
                switch ($_REQUEST['unit_price']) {
                    ...
                    case 5: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_4 . '">';
                        $unitPrice = $unit_price_4;
                        break;
                    case 6: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_5 . '">';
                        $unitPrice = $unit_price_5;
                        break;
                    case 7: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_6 . '">';
                        $unitPrice = $unit_price_6;
                        break;
                }
                echo '</td>
        ...
        if ($result) {
            ...
            while ($row = $result->FetchRow()) {
                ...
                switch ($_REQUEST['unit_price']) {
                    ...
                    case 5: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_4 . '">';
                        $unitPrice = $unit_price_4;
                        break;
                    case 6: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_5 . '">';
                        $unitPrice = $unit_price_5;
                        break;
                    case 7: echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_6 . '">';
                        $unitPrice = $unit_price_6;
                        break;
                }
                echo '</td>
...
//		echo '<tr><td colspan="8">'.$LDNothingtodo.'</td></tr>';
    ...
    function DisplayBillsInfo($bills, $view, $BGCOLOR) {
        ...
        if ($view == "month")
            ...
            echo '<td ' . $BGCOLOR . ' class="td_content"><div align="center" ><a href="billing_tz_archive_date.php?displaybill=true&printout=false&bill_nr=' . $bills['nr'] . '&batch_nr=' . $bills['pid'] . '">' . $bills['nr'] . '</a></div></td>';
        ...
        echo '<td ' . $BGCOLOR . ' class="td_content"><div align="center" >' . $bills['name_last'] . ' ' . $bills['name_first'] . '</div></td>';
    ...
    function getSqlStatment($sql, $max_page_count, $start_timeframe, $end_timeframe, $trans_id, $type, $txtsearch, $sorttyp) {
        ...
        ($this->debug) ? $db->debug = FALSE : $db->debug = FALSE;
        if ($this->debug)
            echo "<br>method::getSqlStatment($sql,$max_page_count,$start_timeframe,$end_timeframe,$trans_id,$type,$txtsearch)<br>";
        $result = $db->Execute($sql);
	...
	<a href="http://www.care2x.org" target=_new>CARE2X 2nd Generation pre-deployment 3.3</a> :: <a href="../../legal_gnu_gpl.htm" target=_new> License</a> :: <a href=mailto:info@care2x.org>Contact</a>  :: <a href="../../language/en/en_privacy.htm" target="pp"> Our Privacy Policy </a> ::
