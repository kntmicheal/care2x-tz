...
class dental extends Core {
    ...
    function BirthDate($pid) {
        global $db;
        //$db->debug=FALSE;
        $this->sql = "SELECT DATE_FORMAT(date_birth,'%d %b %Y') AS birth_date FROM care_person where pid=" . $pid;
        if ($this->result = $db->Execute($this->sql)) {
            if ($this->row = $this->result->FetchRow()) {
                return ($this->row[0]);
            } else {
    ...
    function GetNamesFromBatchNo($bachno) {
        global $db;
        //$db->debug=FALSE;
        $this->sql = "SELECT UPPER(name_last) as name_last, CONCAT(name_first,'  ', name_2) AS name_first FROM care_person where pid=" . $bachno;
        if ($this->result = $db->Execute($this->sql)) {
            if ($this->row = $this->result->FetchRow()) {
                return (ucwords($this->row[1]) . ' ' . ucwords($this->row[0]));
            } else {
                return false;
            }
        } else {
    ...
    function SaveNewPatientNote($xb2) {
        ...
        if ($xb2[6] != '')
            $sql = "UPDATE care_encounter_notes SET " .
                    ...
                    "short_notes = '$xb2[2]', " .
                    "notes = '$xb2[3]', " .
                    "personell_name = '$xb2[4]', " .
                    "date = '" . date('Y-m-d') . "', " .
                    "create_id = '$xb2[4]', " .
                    "create_time = '" . date('Y-m-d H:m:s') . "', " .
                    "modify_time = '" . date('Y-m-d H:m:s') . "', " .
                    "history = " . "'Created: " . date('Y-m-d H:m:s') . " : " . $xb2[4] . "'" .
                    "WHERE nr=" . $xb2[6];
        else
    ...
    function GetPatientNameFromPid($pid) {
        global $db;
        //$db->debug=FALSE;
        $this->sql = "SELECT UPPER(name_last) as name_last, CONCAT(name_first,'  ', name_2) AS name_first FROM care_person where pid=" . $pid;
        if ($this->result = $db->Execute($this->sql)) {
            if ($this->row = $this->result->FetchRow()) {
                echo ucfirst($this->row['name_last']) . ' ' . ucwords($this->row['name_first']);
            } else {
                return false;
            }
        } else {
