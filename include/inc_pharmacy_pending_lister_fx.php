...
function printCheckBox($param, $printout = true) {
    ...
    if ($stored_request[$param] == 1)
        $buffer = '<img ' . createComIcon($root_path, 'chkbox_chk.gif', '0', 'absmiddle', TRUE) . '>';
    else
        $buffer = '<img ' . createComIcon($root_path, 'chkbox_blk.gif', '0', 'absmiddle', TRUE) . '>';
    if ($printout)
        echo $buffer;
    else
        return $buffer;
...
$requests->MoveFirst();
if ($debug)
    echo "back path is:" . $back_path . "<br>";
while ($test_request = $requests->FetchRow()) {
    ...
    if ($batch_nr != $test_request['batch_nr'] || $prescription_date != $test_request['prescribe_date']) {
        ...
        <a onmouseover=\"showBallon('" . $test_request['name_last'] . ", " . $test_request['name_first'] . " encounter: " . $test_request['encounter_nr'] . " Hospital file nr: " . $test_request['selian_pid'] . "',0,150,'#99ccff'); window.status='Care2x Tooltip'; return true;\"
	onmouseout=\"hideBallon(); return true;\" href=\"" . $thisfile . URL_APPEND . "&target=" . $target . "&subtarget=" . $subtarget . "&prescription_date=" . $test_request['prescribe_date'] . "&pn=" . $test_request['encounter_nr'] .
        "&comming_from=" . $comming_from . "&tracker=" . $tracker . "&back_path=" . $back_path . "&prescrServ=prescr&admission=" . $admission . "\">";

        if ($test_request['batch_nr']) {
//            echo $test_request['selian_pid'] . '/' . $test_request['name_last'] . ", " . $test_request['name_first'];
            echo $test_request['selian_pid'];
        }
        echo " " . $test_request['room_nr'] . "/" . $test_request['name_last'] . ", " . $test_request['name_first'] . "</a><br>";
    } else {
