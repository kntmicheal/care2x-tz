...
function getLang($chk_file) {
    ...
    if (file_exists($root_path . 'language/' . $lang . '/lang_' . $lang . '_' . $chk_file)) {
        return 1;
    } else
        return 0;
}
        ...
        if (!preg_match('/edv_system_timeout.php/i', $_SERVER['PHP_SELF'])) {
            ...
            if (!$TIME_OUT_INACTIVE) {
                ...
                if (isset($_SESSION['sess_tos']) || $_SESSION['sess_tos']) {
                    ...
                    if (($tnow - $_SESSION['sess_tos']) >= $TIME_OUT_TIME) {
                        $time_out = TRUE;
                    } else if ($_GET['refreshed'] != '') {
                ...
                //if(!isset($_SESSION['sess_tos'])||!$_SESSION['sess_tos']||!$_SESSION['sess_tos']||$time_out){
...
if (defined('LANG_FILE') && LANG_FILE != '') {
    if (getLang(LANG_FILE)) {
        include($root_path . 'language/' . $lang . '/lang_' . $lang . '_' . LANG_FILE);
    } else {
        include ($root_path . 'language/' . LANG_DEFAULT . '/lang_' . LANG_DEFAULT . '_' . LANG_FILE);
    }
}

# Load additional language tables
if (isset($lang_tables) && is_array($lang_tables) && sizeof($lang_tables)) {
    include_once($root_path . 'include/inc_load_lang_tables.php');
}
...
if (isset($cfg['template_theme']) && !empty($cfg['template_theme'])) {
    $template_theme = $cfg['template_theme'];
}
